...
async def on_chat_message(msg):
    ...
    if text.startswith('/start'):
        await bot.sendMessage(chat_id, 'Deptoon_bot is ready to begin')
    elif text.lower().startswith('/chaqueteardawg'):
        await bot.sendMessage(chat_id, next(dawg_list))


    command = msg['text'][-1:].lower()  # La ultima letra manda el comando

    if command == 'c':
        markup = ReplyKeyboardMarkup(keyboard=[ # botones de ejemplo
            ['Plain text', KeyboardButton(text='Text only')],  # plaintext == keyboardbutton
    ...
    elif command == 'i':  #botones dentro del chat
...
async def on_callback_query(msg):  # respuestas in query
    query_id, from_id, data = telepot.glance(msg, flavor='callback_query')
    print('Callback query:', query_id, from_id, data)

    if data == 'notification':
        await bot.answerCallbackQuery(query_id, text='This is a notification')
    elif data == 'alert':
        await bot.answerCallbackQuery(query_id, text='This is an alert!', show_alert=True)
    elif data == 'edit':
        global message_with_inline_keyboard
